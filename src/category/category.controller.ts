...
export class CategoryController {
    ...
    @ApiQuery({
        ...
        required: false,
        description:
            'true or false for fetching first product for each category',
    })
    async findAll(@Query('q') q: string) {
        ...
        if (includeProduct) {
            categoriesWithFirstProduct = await Promise.all(
                categories.map(async (category) => {
                    const firstProduct =
                        await this.productService.findOneProductByCategoryId(
                            category._id,
                        );
                    return {
                        ...category.toObject(),
            ...
            categoriesWithFirstProduct = categories.map((category) =>
                category.toObject(),
            );
        }
    ...
    @HttpCode(200)
    @ApiOperation({
        summary: 'List of Category / Sub-category and related product.',
    })
    @ApiResponse({
        status: 200,
        description: 'List of Category / Sub-category and related products.',
    })
    async findOne(
        @Param('id') id: string,
        ...
        const { products, total_count } =
            await this.productService.findProductsByCategoryId(id, page, limit);
    ...
    @ApiBearerAuth('access-token')
    @ApiOperation({
        summary: 'Delete Category / Sub-Category and related blogs.',
    })
    @ApiResponse({ status: 200, description: 'Success.' })
    async remove(@Param('id') id: string) {
        ...
        const products =
            await this.productService.findProductIdsByCategoryId(id);
        ...
        return {
            message:
                'Category and related products have been deleted successfully.',
        };
    }
}
