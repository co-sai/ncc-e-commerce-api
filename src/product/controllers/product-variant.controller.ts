...
export class ProductVariantController {
        ...
        schema: {
            ...
            properties: {
                product_id: {
                    type: 'string',
                    nullable: true,
                    example: '6684c64d4c2dd84e8a444528',
                },
    ...
    @UseInterceptors(
        FileFieldsInterceptor([{ name: 'variant_medias', maxCount: 5 }]),
    )
    async createProductVariant(
        ...
        try {
            const variantImagePath = path.join(
                process.cwd(),
                'uploads',
                'product',
                'variants',
            ...
            if (files.variant_medias && files.variant_medias.length > 0) {
                variantImageFileNames = files.variant_medias.map((file) =>
                    path.join(variantImagePath, file.filename),
                );
            }
            ...
            if (body.has_variant.toString() === 'true' && body.variants) {
                ...
                const variants = JSON.parse(body.variants) as Array<{
                    name: string;
                    value: string;
                    price: string;
                    media: string | null;
                ...
                for (let i = 0; i < variants.length; i++) {
                    ...
                    let exVariant =
                        await this.productVariantService.findVariantByName(
                            variant.name,
                        );
                    if (!exVariant) {
                        exVariant =
                    ...
                    if (
                        files.variant_medias &&
                        files.variant_medias.length > 0 &&
                        variant.media.toString() === 'true'
                    ) {
                        ...
                        const newFilename =
                            await this.fileService.generateFileName(
            ...
            const productVariant =
                await this.productVariantService.findProductVariant(
    ...
    async updateSingleVariant(
        ...
        try {
            ...
            const productVariant =
                await this.productVariantService.findProductVariantById(id);
            if (!productVariant) {
                await this.fileService.deleteFiles(variantImageFileNames);
            ...
            if (files.variant_medias && files.variant_medias.length > 0) {
                if (productVariant.media) {
                    await this.fileService.deleteFiles([
                        path.join(process.cwd(), productVariant.media),
                    ]);
                }
