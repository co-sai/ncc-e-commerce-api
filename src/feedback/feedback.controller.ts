...
export class FeedbackController {
    ...
    async feedbackList(@Query() query: { page: string; limit: string }) {
        const page = +query.page || 1;
        const limit = +query.limit || 20;

        const { feedbacks, total_count } =
            await this.feedbackService.findAllFeedback(page, limit);
    ...
    async addFeedback(@Body() body: CreateFeedbackDto) {
